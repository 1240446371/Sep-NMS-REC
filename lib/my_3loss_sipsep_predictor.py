...
# three  : direct cat and then fc and then multipy        
...
class AttVanillaPredictorV2(nn.Module):
    ...
    def __init__(self, att_dropout_p, rank_dropout_p):
        ...
        #print("298......")
        self.rank_fc_gt = nn.Linear(512, 1, bias=True)
        self.rank_fc_fuse = nn.Linear(512, 1, bias=True)
        #print("300......")
    ...
    def forward(self, roi_feat, packed_sent_feat,bert_feats,lfeat):
        ...
        feat_cat_loc = torch.cat([head_pool1, lfeat], 2)  # N R 2048+5 = N R 2053
        feat_cat_loc_map = self.vis_a_fc1(feat_cat_loc) # N , R, 512
        ...
        #weighted_sent_feat = torch.sum(sent_feats * img_to_sent_weight, dim=2)        # [N, R, 512]   this
        # weighted image feat
        img_weighted_by_sent0 = feat_cat_loc_map+feat_cat_loc_map * self.se_img(sent_img0) #[N, R, 512]
        #head_mapped = self.vis_r_fc(feat_cat_loc)
        #t1=0.06
        img_weighted_by_sent = relu(img_weighted_by_sent0)
        #sent_img_merge = relu(sent_img_merge, inplace=True)  # [N, R, 512]
        sent_img_merge = normalize(img_weighted_by_sent, dim=2) # 
        score_gt_neg = self.rank_fc_gt(sent_img_merge)          # [N, R, 1]
        score_gt_neg = score_gt_neg.squeeze(2)               # [N, R]
        ...
        # Compute rank score
        head_mapped = self.vis_r_fc(head_pool)         # [N, R, 512]
        feat_merged0 = head_mapped * sent_feat          # [N, R, 512]
        feat_merged = relu(feat_merged0, inplace=True)  # [N, R, 512]
        feat_merged = normalize(feat_merged, dim=2)    # [N, R, 512]
        # add gt to token ref 
        #feat_merged = torch.cat((feat_merged, sent_img_merge), dim=2)  # 1024
        feat_merged1 = self.rank_dropout(feat_merged)   # [N, R, 512]
        ref_score = self.rank_fc(feat_merged1)          # [N, R, 1]
        ref_score = ref_score.squeeze(2)               # [N, R]
        # add /concat/ last add / before add 
        #img_weighted_by_sent0
        fuse_merge = feat_merged1+sent_img_merge
        fuse_score = self.rank_fc_fuse(fuse_merge) 
        fuse_score = fuse_score.squeeze(2) 
        ...
        return score_gt_neg, ref_score,fuse_score,
