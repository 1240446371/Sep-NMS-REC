...
# first score >class confidence 0.05, then the num =roiperimg , then iou>0.5 is pos then is neg
    ...
    def build_sent_feats(self,tokens, sent):
        ...
        """
        return sentence_embedding,word_embedding, min(len(tokens), self.max_sent_len)+1  # 0 is sent ,and tokens are sentlen
...
class DetEvalLoader:
    ...
    def __iter__(self):
        ...
        for image_id, exps in self.img_to_exps.items():
            ...
            for exp_id, tokens in exps:
                ...
                sent = self.refer.Sents[exp_id]['sent']
                sent_feats,bert_word_feats, sent_len= self.build_sent_feats(tokens,sent)
                sent_feats=sent_feats.unsqueeze(dim=0)
                sent_feats=sent_feats.to(self.device)
                bert_word_feats= bert_word_feats.unsqueeze(dim=1)
                
                bert_word_feats= bert_word_feats.to(self.device)
                sent_len= torch.tensor(sent_len).unsqueeze(dim=0).to(self.device)
                yield exp_id,lfeats, pos_feat, sent_feats, sent_len, bert_word_feats, pos_box, pos_score, cls_num_list
    ...
    def build_sent_feats(self,tokens, sent):
        ...
        # raw_flang = raw_flang.detach()
        """
        return sentence_embedding,word_embedding, min(len(tokens), self.max_sent_len)+1
