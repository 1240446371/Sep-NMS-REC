...
# this  add loc and no phrase norm in cossim_gt
class AttVanillaPredictorV2(nn.Module):
    ...
    def __init__(self, att_dropout_p, rank_dropout_p):
        ...
        self.vis_emb_fc  = nn.Sequential(nn.Linear(2053, 512),
                                     nn.BatchNorm1d(512),
                                     nn.ReLU(),
                                     nn.Dropout(0.1),
                                     nn.Linear(512, 512),
    ...
    def forward(self, roi_feat, packed_sent_feat,bert_feats,lfeat):
        ...
        sent_feat3 = self.phrase_normalizer(sent_feat1)# (N, 512) 
        
        sent_feat3  = sent_feat3.unsqueeze(1).expand(N, R, 512) # (N,R 512)
        sent_feat3 = sent_feat3.contiguous().view(-1, 512) # (N*R, 512) 
        sent_feat3  = sent_feat3.unsqueeze(1).expand(N*R,49, 512) # (N*R,49, 512) 
        sent_feat3 = sent_feat3.contiguous().view(-1, 512) # (N*R*49, 512) 
        #print("153sent_featshape%s"%(sent_feat.shape,))
    
        attn = self.attn_fuse(torch.cat([ head_feat0,sent_feat3], 1)) # (NxRx49, 1)
        ...
        #print("162 lfeat shape %s"%(lfeat.shape,))
        #lfeat = self.loc_normalizer(lfeat)
        lfeat = lfeat.contiguous().view(-1, 5) # (n*R, 5)
        weighted_visual_feats = torch.cat([weighted_visual_feats,lfeat],1)
