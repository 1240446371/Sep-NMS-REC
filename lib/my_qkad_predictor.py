...
class AttVanillaPredictorV2(nn.Module):
    ...
    def forward(self, roi_feat, packed_sent_feat,bert_feats,lfeat):
        ...
        #print("117gt_score%s "%(score_gt_neg,))
        #score_gt_neg = score_gt_neg.squeeze(2) 
        #siggt=torch.sigmoid(score_gt_neg,dim=1)
        #print("120sig_gt%s "%(score_gt_neg,))
        
        #relu_gt = torch.nn.functional.relu(score_gt_neg)
        #print("123 relugt_score%s "%(relu_gt,))
        ...
        #print("158ref_score%s "%(ref_score,))
        #sig_ref= torch.sigmoid( ref_score,dim=1)
        #ref_score
        #ref_score =ref_score + (relu_gt/2)
        ref_score =ref_score + score_gt_neg
        # add loc and gt 
