...
class AttVanillaPredictorV2(nn.Module):
    ...
    def __init__(self, att_dropout_p, rank_dropout_p):
        ...
        self.mapping_word1 = nn.Linear(768,512)

          
        self.norm1 =nn.BatchNorm1d(512)
  
          #nn.BatchNorm1d(512),
        self.mapping_word2 = nn.Sequential(
          nn.ReLU(),
    ...
    def forward(self, roi_feat, sent_len, word_feats,bert_sent_feats,lfeat):
        ...
        word_feats=word_feats.squeeze(dim=1)
        S = word_feats.size(1)
        #sent_len = torch.tensor(sent_len)
        #print("word_feat%s"%(word_feats.shape,))
        #print("793..................sent_len shape %s sentlen%s"%(sent_len.shape,sent_len))
        #print("794..................S%s"%S)
        #print("255........(torch.arange(S) + 1).unsqueeze(dim=0)%s"%((torch.arange(S) + 1).unsqueeze(dim=0).shape,))
        #print("257........sent_len.unsqueeze(dim=1%s"%((sent_len.unsqueeze(dim=0)).shape,))
        print("276.............S%s"%S.is_cuda)
        sent_mask = (torch.arange(S) + 1).unsqueeze(dim=0).expand(N, -1) > sent_len.unsqueeze(dim=1)  # [1,768]
        ...
        #print("282.............word_feats shape%s"%(word_feats.shape,))  # 32,10,768
        word_feats= self.mapping_word1(word_feats) 
        word_feats = torch.transpose(word_feats,1,2)
        word_feats = self.norm1(word_feats)
        word_feats = torch.transpose(word_feats,1,2)
        word_feats = self.mapping_word2(word_feats)
        word_feats = torch.transpose(word_feats,1,2)
        word_feats = self.norm2(word_feats)
        word_feats = torch.transpose(word_feats,1,2)
        word_feats = self.mapping_word3(word_feats)
        #print("746.word_feats%s"%(word_feats.shape,))
        att_value = word_feats.unsqueeze(dim=1).expand(-1, R, -1, -1)  # [N, R, S, 512]
        #att_value = word_feats.expand(-1, R, -1, -1)  # [N, R, S, 512]
        #print("286.............att_value shape%s"%(att_value.shape,))
        ...
        return ref_score
"""
class AttVanillanPredictorV2(nn.Module):

    def __init__(self, att_dropout_p, rank_dropout_p):
        super(AttVanillanPredictorV2, self).__init__()
        # Head network with pretrained weight
        self.head = make_resnet_layer4()
        #self.head1= make_resnet_layer4()
        # Layers of new branch  
    ...
    def forward(self, roi_feat, sent_len, token_feats, bert_sent_feats,lfeat):
        ...
        feat_cat_loc_mapk = self.vis_a_fck(feat_cat_loc) # N , R, 512
        sent_feat_map = self.mapping_sent( bert_sent_feats)# 512
        sent_feat_map= torch.unsqueeze(sent_feat_map,1)
        sent_feat_map0 =sent_feat_map.expand(-1, R, -1) # N R 512
        q_mul_k = feat_cat_loc_mapk * sent_feat_map0  # N R 512 
        #gt_score = torch.sum(q_mul_k, dim=2)# N R    
        gt_score = torch.sum(q_mul_k, dim=2)# N R
            
        #gt_score = torch.sigmoid(gt_score)
        
        token_feats = token_feats.squeeze(dim=1)
        S = token_feats.size(1)
        #sent_len = torch.tensor(sent_len)
        #print("371..........token_feat%s"%(token_feats.shape,))
        #print("793..................sent_len shape %s sentlen%s"%(sent_len.shape,sent_len))
        #print("794..................S%s"%S)
        #print("255........(torch.arange(S) + 1).unsqueeze(dim=0)%s"%((torch.arange(S) + 1).unsqueeze(dim=0).shape,))
        #print("257........sent_len.unsqueeze(dim=1%s"%((sent_len.unsqueeze(dim=0)).shape,))
        ...
        #print("282.............word_feats shape%s"%(word_feats.shape,))  # 32,10,768
        word_feats= self.mapping_word(token_feats) 
        #print("746.word_feats%s"%(word_feats.shape,))
        att_value = word_feats.unsqueeze(dim=1).expand(-1, R, -1, -1)  # [N, R, S, 512]
        #att_value = word_feats.expand(-1, R, -1, -1)  # [N, R, S, 512]
        #print("286.............att_value shape%s"%(att_value.shape,))
        ...
        ref_score = self.rank_fc(feat_merged)          # [N, R, 1]
        ref_score = ref_score.squeeze(2)               # [N, R]       
        ref_score = ref_score+(gt_score)/3

         
        # add loc and gt         

        return gt_score,ref_score
        
class AttVanillanNormFcadPredictorV2(nn.Module):

    def __init__(self, att_dropout_p, rank_dropout_p):
        super(AttVanillanNormFcadPredictorV2, self).__init__()
        # Head network with pretrained weight
        self.head = make_resnet_layer4()
        #self.head1= make_resnet_layer4()
        # Layers of new branch  
    ...
    def forward(self, roi_feat, sent_len, token_feats, bert_sent_feats,lfeat):
        ...
        gt_score = normalize(q_mul_k, dim=2)            # N R 512 
        gt_score = self.rank_gt(gt_score).squeeze(dim=2)  # N R 1--NR             
        #gt_score = torch.sigmoid(gt_score)
        ...
        ref_score = self.rank_fc(feat_merged)          # [N, R, 1]
        ref_score = ref_score.squeeze(2)               # [N, R]
        ref_score =torch.sigmoid(ref_score)
        ref_score = ref_score+(gt_score)/3
        ref_score = torch.clamp(ref_score,max=0.99)
         
        # add loc and gt         

        return gt_score,ref_score

class AttVanillaSigadnolocPredictorV2(nn.Module):

    def __init__(self, att_dropout_p, rank_dropout_p):
        super(AttVanillaSigadnolocPredictorV2, self).__init__()
        # Head network with pretrained weight
        self.head = make_resnet_layer4()
        #self.head1= make_resnet_layer4()
        # Layers of new branch  
    ...
    def forward(self, roi_feat, sent_len, token_feats, bert_sent_feats,lfeat):
        ...
        # Extract visual feature with ResNet Conv-head
        #print("276roishape%s"%(roi_feat.shape,)) # [1, 305, 1024, 7, 7]) N is batch, R is region number   [32, 32, 1024, 7, 7]
        #print("643roi_feat %s sent_len%s token_feats %s  bert_sent_feats %s bert_sent_feats,lfeat %s"%(roi_feat.shape,sent_len.shape,token_feats.shape,bert_sent_feats.shape, lfeat.shape,))
        #  roi [1, 304, 1024, 7, 7]
        #  sent len [1]
        #  token_feats [1, 1, 20, 768]
        # bert_sent: [1, 768]
        # lfeat [1, 304, 5]
        N, R, *_ = roi_feat.shape
        head_feat = self.head(roi_feat.reshape(N * R, 1024, 7, 7)).reshape(N, R, 2048, 7, 7)
        head_pool = head_feat.mean(dim=(3, 4))  # [N, R, 2048]
        #print("643..........sent_len%s"%(sent_len,))
        # Extract word feature with RNN
        #feat_cat_loc = torch.cat([head_pool, lfeat], 2)  # N R 2048+5 = N R 2053
        feat_cat_loc_mapk = self.vis_a_fck(head_pool) # N , R, 512
        sent_feat_map = self.mapping_sent( bert_sent_feats)# [1,512]
        sent_feat_map= torch.unsqueeze(sent_feat_map,1) # [1,1,512]
        sent_feat_map0 =sent_feat_map.expand(-1, R, -1) # N R 512
        q_mul_k = feat_cat_loc_mapk * sent_feat_map0 
        gt_score = torch.sum(q_mul_k, dim=2)# N R         
        gt_score = torch.sigmoid(gt_score)
        
        token_feats = token_feats.squeeze(dim=1) #[1,20,768]
        S = token_feats.size(1) # 
        #sent_len = torch.tensor(sent_len)
        #print("371..........token_feat%s"%(token_feats.shape,))
        #print("793..................sent_len shape %s sentlen%s"%(sent_len.shape,sent_len))
        ...
        #print("257........sent_len.unsqueeze(dim=1%s"%((sent_len.unsqueeze(dim=0)).shape,))
        #print("276.............S%s"%S.is_cuda)
        #print("663 token_feats%s"%(token_feats.shape,))
        
        sent_mask = (torch.arange(S) + 1).unsqueeze(dim=0).expand(N, -1) > sent_len.unsqueeze(dim=1)  # [1,768]
        #print("288pre sent_mask %s"%(sent_mask.shape,))
        sent_mask = sent_mask[:, None, :, None].expand(-1, R, -1, -1)  # [N, R, S, 1]
        #print("667 sent_mask.shape%s"%(sent_mask.shape,))
        #print("290pre sent_mask %s"%sent_mask)
        #print("290pre sent_maskshape %s"%(sent_mask.shape,))
        # Cross-modal attention over words
        #print("741att_key%s  "%(head_pool.shape,))
        ...
        #print("282.............word_feats shape%s"%(word_feats.shape,))  # 32,10,768
        word_feats= self.mapping_word(token_feats)  # 1,20,512
...
# qkad sum, sigmoid add ,clamp
class AttVanillaSigadPredictorV2(nn.Module):

    def __init__(self, att_dropout_p, rank_dropout_p):
        super(AttVanillaSigadPredictorV2, self).__init__()
        # Head network with pretrained weight
        self.head = make_resnet_layer4()
        #self.head1= make_resnet_layer4()
        # Layers of new branch  
...
# qkad sum, sigmoid ,sep loss 
        
class AttVanillaSigsepPredictorV2(nn.Module):

    def __init__(self, att_dropout_p, rank_dropout_p):
        super(AttVanillaSigsepPredictorV2, self).__init__()
        # Head network with pretrained weight
        self.head = make_resnet_layer4()
        #self.head1= make_resnet_layer4()
        # Layers of new branch  
