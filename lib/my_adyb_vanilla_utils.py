...
class DetBoxDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sent_feats = self.build_sent_feats(sent)
        ...
        return roi_feats, roi_labels,labels_gt_neg, word_feats, sent_len,sent_feats,lfeats 
...
# first score >class confidence 0.05, then the num =roiperimg , then iou>0.5 is pos then is neg
    ...
    def build_sent_feats(self, sent):
        ...
        #print("214...........................hidden_states %s"%(hidden_states.shape,))
        #print("215...........................hidden_states %s"%(hidden_states[-1].shape,))
        sentence_embedding = torch.mean(hidden_states[-1], dim=1).squeeze()
        #print("188 sentence_embedding %s"%sentence_embedding)
        #print("189 sentence_embedding size%s" %sentence_embedding.size())
        #Sentence feature at the first position [cls]
...
class DetEvalLoader:
    ...
    IMG_FEAT_DIR="/home_data/wj/matt/cache/feats/refcoco+_unc/mrcn/res101_coco_minus_refer_notime/"  # coco+
    CONF_THRESH = 0.05
    DELTA_CONF = 0.005

    def __init__(self, tokenizer,textmodel,Images,refdb,ctxdb, split='val', gpu_id=0):
    ...
    def __iter__(self):
        ...
        for image_id, exps in self.img_to_exps.items():
            ...
            for exp_id, tokens in exps:
                ...
                ctx_boxes_list= self.exp_to_ctx[str(exp_id)]['ctx'] #lists
             
                ctx_boxes_list1=[]
                for ctx_boxes in ctx_boxes_list:  #keys
                  if ctx_boxes['score']>=0.08: 
                     ctx_boxes_list1.append(ctx_boxes)
                exp_to_ctx1[exp_id] = {'ctx': ctx_boxes_list1}
                #print("240exp_to_ctx1%s"%(exp_to_ctx1,))  
                rpn_clip_score = self.get_rpn_score_clip(image_id, exp_to_ctx1[exp_id],pos_box) 
                
                
                
                yield exp_id,lfeats, pos_feat, sent_feat,bert_feats, pos_box, pos_score, cls_num_list,rpn_clip_score
    ...
    def get_rpn_score_clip(self, image_id, exp_to_ctx_list1,pos_box):
      
        # for box in boxes[top_idx]:
        ctx_boxes_list= exp_to_ctx_list1['ctx']
        #print("255pos_box.shape(0)%s"%(type(pos_box.shape)))
        rpn_clip_score = np.zeros(pos_box.shape[0])
        
        
        rpn_num = 0
        for rpn_box in pos_box:  
 
            for ann_ctx_box in ctx_boxes_list:
               if calculate_iou(rpn_box, ann_ctx_box['box']) >= 0.5:
                   rpn_clip_score[rpn_num] = ann_ctx_box['score']
                   #print("265rpn_clip_score%s"%rpn_clip_score[rpn_num])
                   break
            rpn_num+=1   
           
    
       
        return rpn_clip_score
        
        
        
class DetEvalLoader1:
