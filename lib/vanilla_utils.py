...
class DetBoxDataset(Dataset):
    ...
    #HEAD_FEAT_DIR= '/home/wj/code/MAttNet_master/cache/feats/refcoco_unc/mrcn/res101_coco_minus_refer_notime_ann_feats.h5'
    #HEAD_FEAT_DIR= '/home/wj/code/MAttNet_master/cache/feats/refcoco_unc/mrcn/res101_coco_minus_refer_notime/'
    HEAD_FEAT_DIR= "/data1/wj/matt/cache/feats/refcocog_umd/mrcn/res101_coco_minus_refer_notime/"  # this
    ...
    def __init__(self, refdb, ctxdb, split, roi_per_img):
        ...
        with open(self.BOX_FILE_PATH, 'rb') as f:
            self.img_to_det_box = pickle.load(f)  # load all mrcn rpn box 
        with open(self.SCORE_FILE_PATH, 'rb') as f:
            self.img_to_det_score = pickle.load(f)  # load all mrcn box score
        self.idx_to_glove = np.load('/home_data/wj/ref_nms/cache/std_glove_{}.npy'.format(refdb['dataset_splitby']))  # load glove feats or glove
        self.max_sent_len = 20 if refdb['dataset_splitby'] == 'refcocog_umd' else 10  # why load sent len 
        self.pad_feat = np.zeros(300, dtype=np.float32)
        # Number of samples to draw from one image
        self.roi_per_img = roi_per_img  # 32 box per image
    ...
    def __getitem__(self, idx):
        ...
        #print("76 vanroilabel%s"%roi_labels.shape)  # 32 
        #print("77 van roilabel%s"%roi_labels)
        #print("78van samplepos%s neg%s"%(len(sampled_pos),len(sampled_neg)))
...
# first choice score >class confidence 0.05, then the num =roiperimg , then iou>0.5 is pos then is neg
    def get_labeled_rois(self, image_id, target_list):
        boxes = self.img_to_det_box[image_id].reshape(-1, 81, 4)
        #print("88box%s"%(boxes.shape,))# (300,81,4)(159,81,4)
        ...
        this_thresh = self.CONF_THRESH  # 0.05 is class confidence from mrcn
        positive = scores > this_thresh # list of [true ...,false...] 
        #print("95positive%s"%positive)
        while np.sum(positive) < self.roi_per_img:  # maybe > roi_per img
            this_thresh -= self.DELTA_CONF
            positive = scores > this_thresh
        pos_rois = []    
        neg_rois = []
        # for box in boxes[top_idx]:
        #print("107 vanpos len%s "% np.sum(positive))  # first class threshold 
        for box in boxes[positive]:
            #print("104boxpositive%s"%positive)
            #print("105boxpositive%s"%box)
    ...
    def build_word_feats(self, tokens):
        ...
        #print("van122wordfeats%s"%word_feats)
        #print("van123len(word_feats)%s"%len(word_feats))  # list and len is token_num
        word_feats += [self.pad_feat] * max(self.max_sent_len - len(word_feats), 0)  # pad to 10
        #print("van125 +len(word_feats)%s"%len(word_feats))  # 
        word_feats = torch.tensor(word_feats[:self.max_sent_len])  # [S, 300] 10 *300
        #print("van127 word_feats%s"%(word_feats.shape,))
...
class DetEvalLoader:
    ...
    #IMG_FEAT_DIR = "/data1/wj/matt/cache/feats/refcocog_umd/mrcn/res101_coco_minus_refer_notime/"
    CONF_THRESH = 0.05
    DELTA_CONF = 0.005
