...
class DetBoxDataset(Dataset):
    ...
    #HEAD_FEAT_DIR= '/home/wj/code/MAttNet_master/cache/feats/refcocog_umd/mrcn/res101_coco_minus_refer_notime/' # 210
    HEAD_FEAT_DIR= "/data1/wj/matt/cache/feats/refcocog_umd/mrcn/res101_coco_minus_refer_notime/" # 18
    ...
    def __init__(self, tokenizer,textmodel,dataset,split_by,Images,refdb, ctxdb, split, roi_per_img):
        ...
        self.max_sent_len = 40 if refdb['dataset_splitby'] == 'refcocog_umd' else 20  # i change it
    ...
    def __getitem__(self, idx):
        ...
        bert_sent_feats,bert_token_feats, sent_len= self.build_sent_feats(ref['tokens'],sent)
        ...
        bert_sent_feats = bert_sent_feats.detach()
        #bert_word_feats = bert_word_feats.squeeze(dim=1)
        bert_token_feats = bert_token_feats.detach()
        ...
        return roi_feats, roi_labels,labels_gt_neg, sent_len, bert_token_feats, bert_sent_feats,lfeats  # bert word
...
# first score >class confidence 0.05, then the num =roiperimg , then iou>0.5 is pos then is neg
    ...
    def build_sent_feats(self,tokens, sent):
        ...
        #token_len = sent_tokenize['input_ids'].squeeze(dim=1).size(1)-2
        #token_len= torch.count_nonzero(sent_tokenize['input_ids'], dim=1).item()
        token_len=0
        for i in range(self.max_sent_len):
            if sent_tokenize['input_ids'][:,i]>0:
                token_len+=1
        token_len=token_len-2  # cls and sep
        ...
        token_embedding = last_hidden_state[:, 1:, :]  #[b,len,512]
        ...
        """
        return sentence_embedding,token_embedding,token_len
...
class DetEvalLoader:
    ...
    #IMG_FEAT_DIR = '/home/wj/code/MAttNet_master/cache/feats/refcoco+_unc/mrcn/res101_coco_minus_refer_notime/' #210
    #IMG_FEAT_DIR="/home_data/wj/matt/cache/feats/refcoco+_unc/mrcn/res101_coco_minus_refer_notime/"  # coco+ 18
    IMG_FEAT_DIR= '/home/wj/code/MAttNet_master/cache/feats/refcoco_unc/mrcn/res101_coco_minus_refer_notime/'
    ...
    def __init__(self, tokenizer,textmodel,Images,refdb, split='val', gpu_id=0):
        ...
        self.refer = REFER('/home_data/wj/ref_nms/data/refer/', self.dataset_, self.split_by)
        #self.refer = REFER('/home_data/wj/code/ref_nms/data/', self.dataset_, self.split_by)
    ...
    def __iter__(self):
        ...
        for image_id, exps in self.img_to_exps.items():
            ...
            for exp_id, tokens in exps:
                ...
                sent = self.refer.Sents[exp_id]['sent']
                bert_sent_feats,bert_token_feats, token_len= self.build_sent_feats(tokens,sent)
                bert_token_feats= bert_token_feats.unsqueeze(dim=1)
                bert_token_feats= bert_token_feats.to(self.device) 
                token_len= torch.tensor(token_len).unsqueeze(dim=0)
                bert_sent_feats= torch.tensor(bert_sent_feats).unsqueeze(dim=0).to(self.device)
                #print("369...")
                yield exp_id, pos_box, pos_score,pos_feat,  token_len, bert_token_feats,bert_sent_feats, lfeats, cls_num_list
