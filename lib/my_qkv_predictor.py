...
class AttVanillaPredictorV2(nn.Module):
    ...
    def __init__(self, att_dropout_p, rank_dropout_p):
        ...
        self.gt_dropout =  nn.Dropout(p=rank_dropout_p, inplace=True)
        #print("298......")
    ...
    def forward(self, roi_feat, packed_sent_feat,bert_feats,lfeat):
        ...
        q_mul_k = torch.softmax(q_mul_k,dim=2) # N R 512 --att 1 softmax(512)
        feat_cat_loc_mapv = self.vis_a_fcv(feat_cat_loc)
        #sent_img_qkv= torch.sum(q_mul_k*feat_cat_loc_mapv,dim=2) # N R 512
        
        sent_img_qkv= q_mul_k*feat_cat_loc_mapv  # N R 512 weighted image
        sent_img_qkv = sent_feat_map0 * sent_img_qkv  # N R 512 
        sent_img_qkv = relu(sent_img_qkv, inplace=True)
        sent_img_qkv = normalize(sent_img_qkv, dim=2)    # [N, R, 512]
        
        sent_img_qkv1 = self.gt_dropout(sent_img_qkv)
        score_gt_neg = self.rank_fc_gt(sent_img_qkv1)          # [N, R, 1]
        
        score_gt_neg = score_gt_neg.squeeze(2)               # [N, R]
        score_gt_neg = score_gt_neg/0.06
