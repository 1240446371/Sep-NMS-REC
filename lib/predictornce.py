...
class AttVanillaPredictor(nn.Module):
    ...
    def forward(self, roi_feat, ctx_pos_num, neg_num, packed_sent_feat):
        """
...
class AttVanillaPredictorV2(nn.Module):
    ...
    def forward(self, roi_feat, gt_num,ctx_pos_num, neg_num, packed_sent_feat):
        """
        Args:
            roi_feat: [N, R, 1024, 7, 7].
            packed_sent_feat: `PackedSequence` object, should be moved to proper device in advance.
        ...
        #t0=0.2
        #t1=0.3
        #print("gt_num%s,ctxnum%s,negnum%s"%(gt_num.shape,ctx_pos_num.shape,neg_num.shape))
        #score_gt_ctx_neg0 = (head_mapped*sent_feat)
        score_gt_ctx_neg0 =torch.einsum("nrc,nrc->nr",head_mapped,sent_feat)
        score_gt_ctx_neg = score_gt_ctx_neg0/t0
        score_ctx_neg = score_gt_ctx_neg0/t1
        #print("317score_gt_ctx_neg%s"%(score_gt_ctx_neg.shape,))
        #print("318score_ctx_neg%s"%(score_ctx_neg.shape,))
        #print("320 score_ctx_neg%s"%(score_ctx_neg.device))
        ...
        return score_gt_ctx_neg,score_ctx_neg,ref_score,
