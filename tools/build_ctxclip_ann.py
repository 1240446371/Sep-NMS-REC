...
#.397299999997, 'iscrowd': 0, 'image_id': 579382, 'bbox': [416.73, 78.27, 63.27, 447.12], 'category_id': 1, 'id': 201995}
                ...
                if len(noun_tokens)>0:
                    for ann in refer.imgToAnns[image_id]:  # ann has many 7 annotations include cat_id 
                        ...
                        im_box0 = im[y1:y2,x1:x2]
                        im_box=Image.fromarray(im_box0)
                    ...
                    if clip_soft_to_array.size==len(clip_soft.t()):
                        ...
                        for item in clip_soft:
                            ...
                            i+=1
                        result_path = '/data1/wj/ref_nms/result/result04/{}_{}'.format(str(ref_id),(str(image_id).zfill(12)+'.jpg'))
                        ...
                        i = 0
                        for itemi in  clip_soft:
                                j = 0
                                #print("181 itemi%s"%itemi)
                                plt.yticks([0], [noun_tokens[i]])  # i is the num of token
                                #ax.text(draw_box[2], draw_box[3],str(noun_tokens[j]))
                                for itemj in itemi:
                                    draw_box = ann_box_list[j]
                                    box_plot = Rectangle((draw_box[0], draw_box[1]), draw_box[2]-draw_box[0]+1,draw_box[3]-draw_box[1]+1, fill=False, edgecolor='blue', linewidth=1)
                                    ax.add_patch(box_plot)
                                    ax.text(draw_box[0], draw_box[1], '{:.1f}'.format(itemj.item()), bbox={'facecolor':'white', 'alpha':0.5})
                                    #ax.text(draw_box[0], draw_box[1], '{:.1f}'.format(itemj.item()))

                                    if itemj >=0.4:
                                    # draw
                                        if calculate_iou(ann_box_list[j], gt_box) > 0.9:  # if iou>0.9 is gt,else is 
                                            gt_hit = True
                                        else:
                                            ctx_list.append({'box': ann_box, 'cat_id': ann['category_id']})
                                    j+=1
                                # save this token 
                                result_path = '/data1/wj/ref_nms/result/result04/{}_{}_{}'.format(str(ref_id),str(noun_tokens[i]),(str(image_id).zfill(12)+'.jpg'))
                                plt.savefig(result_path) 
                                ax.set_axis_off()
                                ax.set_yticks([])
                                ax.set_xticks([])
                                
                                # plt image for this ref's next token 
                                plt.figure()
                                ax = plt.gca()
                                ax.imshow(I)
                                box_plot = Rectangle((gt_box[0], gt_box[1]), gt_box[2]-gt_box[0]+1,gt_box[3]-gt_box[1]+1, fill=False, edgecolor='green', linewidth=3)
        ...
        save_path1  = '/data1/wj/ref_nms/cache/clip_ctx_04info{}.json'.format(dataset_splitby)
    ...
    # Save results
    save_path = '/data1/wj/ref_nms/cache/clip_ctxdb_ann_04soft{}.json'.format(dataset_splitby)
