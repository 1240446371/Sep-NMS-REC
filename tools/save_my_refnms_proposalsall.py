   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,lfeats, pos_feat, sent_feat, bert_feats,pos_box, pos_score, cls_num_list in tqdm_loader:
        ...
        for cls_gt, cls_ref, cls_pos_box, cls_pos_score in zip(gt_score_list,rank_score_list,pos_box_list, pos_score_list):
            ...
            cls_ref_adgt01 =final_score_ref + final_score_gt01
            cls_ref_adgtall =final_score_ref + final_score_gt
            

            # ref ad gt nms  
            keepad01 = nms(cls_pos_box, cls_ref_adgt01, iou_threshold=0.3)
            keepadall = nms(cls_pos_box, cls_ref_adgtall, iou_threshold=0.3)  
          


            cls_kept_box_adgt01 = cls_pos_box[keepad01]
            cls_kept_score_adgt01 = cls_ref_adgt01[keepad01] 
            cls_kept_box_adgtall = cls_pos_box[keepadall]
            cls_kept_score_adgtall = cls_ref_adgtall[keepadall]              

            # gt nms
            keepgt01 = nms(cls_pos_box, final_score_gt01, iou_threshold=0.3)
 
             
            # (ref + gtnms)*cls  then  nms 
            final_score_ref0= final_score_ref
  
            
            cls_kept_score_gtnms01 = final_score_gt01[keepgt01]
            final_cls_ref_ad_gtnms01 =  final_score_ref0.scatter_add_(0, keepgt01, cls_kept_score_gtnms01)
            
            keep_ad_nms01 = nms(cls_pos_box, final_cls_ref_ad_gtnms01, iou_threshold=0.3)
            cls_kept_box_nms01 = cls_pos_box[keep_ad_nms01]
            cls_kept_score_nms01 = final_cls_ref_ad_gtnms01[keep_ad_nms01]     
            
            for box2, score2 in zip(cls_kept_box_adgt01, cls_kept_score_adgt01):
                proposals_adgt01.append({'score': score2.item(), 'box': box2.tolist(), 'cls_idx': cls_idx})
            for box3, score3 in zip(cls_kept_box_adgtall, cls_kept_score_adgtall):
                proposals_adgtall.append({'score': score3.item(), 'box': box3.tolist(), 'cls_idx': cls_idx})
            for box4, score4 in zip(cls_kept_box_nms01, cls_kept_score_nms01):
                proposals_adgtnms.append({'score': score4.item(), 'box': box4.tolist(), 'cls_idx': cls_idx})
        ...
        exp_to_proposals_adgt01[exp_id] = proposals_adgt01
        exp_to_proposals_adgtall[exp_id]= proposals_adgtall
        exp_to_proposals_adgtnms[exp_id]= proposals_adgtnms
    ...
    return (exp_to_proposals_gt,exp_to_proposals_ref,exp_to_proposals_adgt01,exp_to_proposals_adgtall,exp_to_proposals_adgtnms)
...
def main(args):
    ...
    save_path3 = '/data1/wj/ref_nms/cache/my_att_qkvcat_adgtall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path4 = '/data1/wj/ref_nms/cache/my_att_qkvcat_adgtnms_{}_{}.pkl'.format(args.m, args.dataset)
    #save_path5 = '/data1/wj/ref_nms/cache/0my_refsipsep_adgt05nms_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path0 = '/data1/wj/ref_nms/cache/my_att_qkvcat_gt_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_att_qkvcat_ref_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path2 = '/data1/wj/ref_nms/cache/my_att_qkvcat_adgt01_proposals_{}_{}.pkl'.format(args.m, args.dataset)
