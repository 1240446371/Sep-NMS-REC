...
from lib.my_2bert_vanilla_utils import DetEvalLoader
...
from lib.my_2bert_predictor import AttVanillaPredictorV2
...
def rank_proposals(position, gpu_id, tid, refdb_path, split, m,dataset_splitby ):
   ...
   # device = torch.device('cuda', gpu_id)
    ...
    model_path = "/home_data/wj/ref_nms/output/my_2bert_attvanilla_refcoco+_unc_1031122145_b.pth"
    ...
    for exp_id,lfeats, pos_feat, bert_sent_feats,sent_len, bert_word_feats,pos_box, pos_score, cls_num_list in tqdm_loader:
        ...
        #print("pos_feat, %s sent_len %s ,bert_word_feats %s ,bert_sent_feats %s ,lfeats %s pos_score,%s pos_box %s"%(pos_feat.is_cuda,sent_len.is_cuda,bert_word_feats.is_cuda,bert_sent_feats.is_cuda,lfeats.is_cuda,pos_score.is_cuda,pos_box.is_cuda))
        with torch.no_grad():
            #print("131....................")
            rank_score = predictor(pos_feat, sent_len,bert_word_feats,bert_sent_feats,lfeats)  # [1, n_box]
        
        #print("134......rank_score%s"%rank_score.is_cuda)
        ...
        cls_idx = 0
        for cls_ref, cls_pos_box, cls_pos_score in zip(rank_score_list,pos_box_list, pos_score_list):
            cls_idx += 1
            if cls_ref.size(0) == 0: # size(0)some is 0, and some is 16 or other
                continue
            # gt cls
            ...
            for box1, score1 in zip(cls_kept_box_ref, cls_kept_score_ref):
                proposals.append({'score': score1.item(), 'box': box1.tolist(), 'cls_idx': cls_idx})
        
        assert cls_idx == 80

        exp_to_proposals_ref[exp_id]= proposals
        
 
    return exp_to_proposals_ref
...
def main(args):
    ...
    save_path = '/data1/wj/ref_nms/cache/my_2bert_proposals_{}_{}.pkl'.format(args.m, args.dataset)
