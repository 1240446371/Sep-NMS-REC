...
def main(args):
    ...
    #clipfeats_token = f.create_dataset('clipfeats_token', (num_dets, 512), dtype=np.float32)
        ...
        for image_id, exps in img_to_exps.items(): # all express
                ...
                for exp_id,_  in exps:        
                     ...
                     for token in tokens:
                          ...
                          #jit_model = jit_model.float()
                          _,_,token_feat,_, _ = jit_model(img_, re)
                          token_feat =torch.squeeze(token_feat).float()
                          for n in range(pos_box.shape[0]):
                               ...
                               img_feats = torch.from_numpy(np.array(img_feats)).to(device_cuda).float()  # list to tensor
                               # get  token feats
                               ...
                               #print("157logit_scale%s"%logit_scale)
                               #print("178type%s"%type(logit_scale))
                               #print("159tokenfeat%s"%(token_feat))
                               #print("160imgfeat%s token_feat%s"%(img_feats.shape,token_feat.shape))
                               #cos_sim = torch.cosine_similarity(img_feats,token_feat,dim=0)
                               #print("164cos sim %s"%cos_sim)
                               logits = logit_scale * img_feats @ token_feat.t()  # logit_scale: tesnor
                          ...
                          rank_score_norm_list = F.softmax(rank_score_norm)
        ...
        pro_save_path = '/data1/wj/ref_nms/cache/clippro_alltoken_soft_{}_{}.pkl'.format( dataset_splitby,split)
