...
def compute_loss(predictor, criterion, device, enable_grad, roi_feats, roi_labels, word_feats, sent_len):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        #print("train 76 roi_feats %s"% (roi_feats.shape,)) # ([32, 32, 1024, 7, 7])
        #print("train 77 word_feats %s"% (word_feats.shape,)) # torch.Size([32, 10, 300])  batch = 32, len =10, feat len =300 300d glove
        #print("train 78 packed_sent_feats %s"% (packed_sent_feats.shape,))  wrong
        print("78maxscoresshape %s"%(scores.shape,))  # 32*32
...
def main(args):
    ...
    refdb_path = '/home_data/wj/ref_nms/cache/std_refdb_{}.json'.format(dataset_splitby)  # gtbox
    print('loading refdb from {}...'.format(refdb_path))
    with open(refdb_path, 'r') as f:
        refdb = json.load(f)
    #ctxdb_path = '/home_data/wj/ref_nms/cache/std_ctxdb_{}.json'.format(dataset_splitby) # ctxbox
    ...
    for epoch in range(CONFIG['EPOCH']):
        for trn_batch in trn_loader:
            ...
            step += 1
            predictor.train()
            loss = compute_loss(predictor, criterion, device, True, *trn_batch)  # predictor:max_ref_score, max_idx, cls_score, bbox_pred
        ...
        save_path = '/home_data/wj/ref_nms/output/testatt_vanilla_ckpt_{}_{}.pth'.format(tid, epoch + 1)
    ...
    save_path = '/home_data/wj/ref_nms/output/testatt_vanilla_{}_b.pth'.format(tid)
    ...
    with open('/home_data/wj/ref_nms/output/testatt_vanilla_{}.json'.format(tid), 'w') as f:
