   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,pos_box, pos_score, pos_feat, sent_len, bert_token_feats,bert_sent_feats, lfeats, cls_num_list in tqdm_loader:
        ...
        for cls_gt, cls_ref, cls_pos_box, cls_pos_score in zip(gt_score_list, rank_score_list,  pos_box_list, pos_score_list):
            ...
            if str(exp_id) in coco_out[split].keys() :  # coco out---> gt is import
                #print("162...not in .%s")
            ...
            else:  # coco in, cls is import
                #print("173.esp id ..................in")
                final_score_gt = cls_gt * (cls_pos_score)**(2)
                keepgt = nms(cls_pos_box, final_score_gt, iou_threshold=0.3)
                #keepref = nms(cls_pos_box, final_score_ref, iou_threshold=0.3)
                cls_kept_box_gt = cls_pos_box[keepgt] 
                cls_kept_score_gt = final_score_gt[keepgt]
            ...
            if str(exp_id) in coco_out[split].keys() :  # cocoout ---> gt is import
            
                final_score_gt2 = cls_gt * ((cls_pos_score)**(1/2))
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_Sigad_cocoin_clssquar_bertall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_Sigad_cocoin_clssquar_out_clssqrt_bertall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
