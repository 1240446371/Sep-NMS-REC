...
from lib.my_bertall_predictor import AttVanillaSigadnolocPredictorV2
...
def rank_proposals(position, gpu_id, tid, refdb_path, split, m,dataset_splitby ):
   ...
   # device = torch.device('cuda', gpu_id)
    ...
    predictor = AttVanillaSigadnolocPredictorV2(att_dropout_p=model_info['config']['ATT_DROPOUT_P'],
    ...
    #model_path = "/home_data/wj/ref_nms/output/my_bertall_qkad_sigad_refcoco_unc_1121163854_b.pth"
    #model_path = "/home_data/wj/ref_nms/output/my_bertall_qkad_refcoco+_unc_1113164944_b.pth"
    model_path ="/home_data/wj/ref_nms/output/my_bertall_qkad_sigad_noloc_refcoco+_unc_1127213725_b.pth"
    ...
    for exp_id,pos_box, pos_score, pos_feat, sent_len, bert_token_feats,bert_sent_feats, lfeats, cls_num_list in tqdm_loader:
        ...
        #print("130.........")
        gt_score_list = torch.split(gt_score[0], cls_num_list, dim=0)  # tuple       
        ...
        cls_idx = 0
        for cls_gt, cls_ref, cls_gt_ad_ref, cls_pos_box, cls_pos_score in zip(gt_score_list, rank_score_list, gt_ad_ref_list, pos_box_list, pos_score_list):
            #print("144ok")
            cls_idx += 1
            if cls_ref.size(0) == 0 and cls_gt.size(0) == 0 and cls_gt_ad_ref.size(0) == 0: # size(0)some is 0, and some is 16 or other
                continue
            # gt cls
            ...
            for box2, score2 in zip(cls_kept_box_gt, cls_kept_score_gt):
                proposals_gt.append({'score': score2.item(), 'box': box2.tolist(), 'cls_idx': cls_idx})
                
            
            final_score_adgtall = cls_gt_ad_ref * cls_pos_score
            keepadgtall = nms(cls_pos_box,final_score_adgtall, iou_threshold=0.3)
            #keepref = nms(cls_pos_box, final_score_ref, iou_threshold=0.3)
            cls_kept_box_adall = cls_pos_box[keepadgtall]
            cls_kept_score_adall = final_score_adgtall[keepadgtall]
            for box3, score3 in zip(cls_kept_box_adall,cls_kept_score_adall):
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_bertall_qkadnoloc_gt_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_bertall_qkadnoloc_ref_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path2 = '/data1/wj/ref_nms/cache/my_bertall_qkadnoloc_adgtall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tid', type=str, default='1121163854')# coco+ 0112095046 0318182906
