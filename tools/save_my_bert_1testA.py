...
from utils.constants import EVAL_SPLITS_DICT1
...
def rank_proposals(position, gpu_id, tid, refdb_path, split, m,dataset_splitby ):
   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,pos_box, pos_score, pos_feat, sent_len, bert_token_feats,bert_sent_feats, lfeats, cls_num_list in tqdm_loader:
        ...
        print("gtscore %s"%(gt_score[0].shape,)) # nobx
        ...
        for cls_gt1,cls_pos_box, cls_pos_score in zip(gt_score_list,pos_box_list, pos_score_list):
            ...
            if cls_gt1.size(0) == 0 : # size(0)some is 0, and some is 16 or other
                continue
            # gt cls
            idx = cls_pos_score>=0.3
            #print("168,,,,,cls_pos_box%s"%(cls_pos_box.shape,))
            #print("169,,,,,cls_gt1%s"%(cls_gt1.shape,))
            print("171,,,,idx%s"%(idx.shape,))
            cls_gt_pos = cls_gt1[idx]
            cls_pos_box1=cls_pos_box[idx]
            print("171,,,,,cls_gt1%s"%(cls_gt_pos.shape,))
            #final_score_gt1 = cls_gt1 * cls_pos_score
            
                
            cls_pos_score1 = cls_pos_score[idx]            
            final_score_gt1 = cls_gt_pos + (cls_pos_score1/30)
            keepgt2 = nms(cls_pos_box1, final_score_gt1, iou_threshold=0.3)
            #keepref = nms(cls_pos_box, final_score_ref, iou_threshold=0.3)
            cls_kept_box_gt2 = cls_pos_box1[keepgt2]
            cls_kept_score_gt2 = final_score_gt1[keepgt2]
            ...
            final_score_gt3 = cls_gt_pos* (1+cls_pos_score1)
            keepgt3 = nms(cls_pos_box1, final_score_gt3, iou_threshold=0.3)
            #keepref = nms(cls_pos_box, final_score_ref, iou_threshold=0.3)
            cls_kept_box_gt3 = cls_pos_box1[keepgt3]
        ...
        exp_to_proposals_gt1[exp_id]= proposals_gt2
        exp_to_proposals_gt2[exp_id]= proposals_gt3
...
def main(args):
    ...
    eval_splits = EVAL_SPLITS_DICT1[dataset_splitby]
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_Sigad_cls0.3_gt+clsc30_testAproposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_Sigad_cls0.3_gt(1+cls)_testAproposals_{}_{}.pkl'.format(args.m, args.dataset)
