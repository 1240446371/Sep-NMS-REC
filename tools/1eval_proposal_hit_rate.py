...
def threshold_with_confidence(exp_to_proposals, conf):
    ...
    #for exp_id, proposals in exp_to_proposals:
        ...
        for prop in sorted_proposals[1:]:
            if prop['score'] > conf:  # to choice score
...
def main(args):
    ...
    split = 'testB'
    proposal_path = '/data1/wj/ref_nms/cache/clippro_alltoken_nocls_{}_{}.pkl'.format( dataset_splitby,split)
    ...
    evaluator = NewHitRateEvaluator(refer, top_N=10, threshold=args.thresh)
    ...
    #for exp_id, proposals in exp_to_proposals:
        #print("exp_to_proposals.items%s"%(exp_to_proposals.items(),))
        if len(proposals)== 0:
            print("58exp_id%s proposal %s"%(exp_id,proposals))
            continue 
        # assert len(proposals) >= 1
        sorted_proposals = sorted(proposals, key=lambda p: p['score'], reverse=True)
        thresh_proposals = [sorted_proposals[0]]
    ...
    proposal_per_ref, hit_rate = evaluator.eval_hit_rate(split, results)
    ...
    for split in eval_splits:
        ...
        #print("54exp_to_proposals%s"%(exp_to_proposals.items(),))
