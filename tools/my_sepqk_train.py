...
from lib.my_sep_qktrain_predictor import AttVanillaPredictorV2
...
def init_att_vanilla_predictor(predictor):
    ...
    predictor.head1.load_state_dict(c4_weights)
...
def compute_loss(predictor, criterion, device, enable_grad, roi_feats, roi_labels,labels_gt_neg, word_feats, sent_len,sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        scores_gt_neg, = predictor.forward(roi_feats, sent_feats ,lfeats)  # max_ref_score, max_idx, cls_score, bbox_pred
        #print("78maxscoresshape %s"%scores.shape)
        #labels_gt_neg=labels_gt_neg.to(device)
        #loss0= criterion(score_gt.flatten(), labels_gt.flatten())
        loss= criterion(scores_gt_neg.flatten(), labels_gt_neg.flatten())
...
def main(args):
    ...
    #ctxdb_path ="/home/wj/code/ref_nms/cache/std_ctxdb_refcoco+_unc.json"
    ...
    # Setup optimizer
    ref_params = list(predictor.vis_a_fck.parameters())+list(predictor.mapping_sent.parameters())
    #print("131........")
    ...
    head1_optimizer = optim.Adam(predictor.head1.parameters(), lr=CONFIG['HEAD_LR'], weight_decay=CONFIG['HEAD_WD']) # add
    ...
    head1_scheduler = optim.lr_scheduler.ReduceLROnPlateau(head1_optimizer, min_lr=CONFIG['HEAD_LR']/100, **common_args) # ade
    # Start training
    step = 0
    trn_running_loss = 0.
    ...
    for epoch in range(CONFIG['EPOCH']):
        ...
        epoch_ckpt = {
            ...
            'head1_optimizer': head1_optimizer.state_dict(),
            'ref_scheduler': ref_scheduler.state_dict(),
            #'rnn_scheduler': rnn_scheduler.state_dict(),
            'head1_scheduler': head1_scheduler.state_dict(),
    ...
    save_path = '/home_data/wj/ref_nms/output/my_sepqkad_train_{}_{}_b.pth'.format(args.dataset,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_sepqkad_train_{}_{}.json'.format(args.dataset,tid), 'w') as f:
