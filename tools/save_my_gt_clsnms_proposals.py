   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,lfeats, pos_feat, sent_feat, bert_feats,pos_box, pos_score, cls_num_list in tqdm_loader:
        ...
        for cls_gt, cls_ref, cls_pos_box, cls_pos_score in zip(gt_score_list,rank_score_list,pos_box_list, pos_score_list):
            ...
            cls_score_001 = torch.where(cls_gt < 0.01, zero, cls_pos_score)  #  gt<001 and cls is 0 
            cls_pos_score0 = cls_score_001 
            cls_pos_score0.scatter_add_(0,keepgt,tag)

            #print("zero%s"%zero)
            """
            keepcls = nms(cls_pos_box, cls_pos_score0, iou_threshold=0.3)
            cls_kept_box_cls0 = cls_pos_box[keepcls]
            cls_kept_score_cls0 = cls_pos_score0[keepcls]
      
          
            for box0, score0 in zip(cls_kept_box_cls0,cls_kept_score_cls0):
                proposals_gt.append({'score': score0.item(), 'box': box0.tolist(), 'cls_idx': cls_idx}) 

            """    
                
            cls_score_002 = torch.where(cls_score_001 < 0.2, zero, cls_score_001)
            cls_pos_score1 = cls_score_002
            cls_pos_score1.scatter_add_(0,keepgt,tag)
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_qkad_gt001to0_nms02_adcls_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_qkad_gt001to0_nms_adcls02to0_proposals_{}_{}.pkl'.format(args.m, args.dataset)
