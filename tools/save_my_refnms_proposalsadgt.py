   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,lfeats, pos_feat, sent_feat, bert_feats,pos_box, pos_score, cls_num_list in tqdm_loader:
        ...
        for cls_gt, cls_ref, cls_pos_box, cls_pos_score in zip(gt_score_list,rank_score_list,pos_box_list, pos_score_list):
            ...
            for box0, score0 in zip(cls_kept_box_adgt01, cls_kept_score_adgt01):
                proposals_adgt01.append({'score': score0.item(), 'box': box0.tolist(), 'cls_idx': cls_idx})
            for box1, score1 in zip(cls_kept_box_adgtall, cls_kept_score_adgtall):
                proposals_adgtall.append({'score': score1.item(), 'box': box1.tolist(), 'cls_idx': cls_idx})
            for box2, score2 in zip(cls_kept_box_nms01, cls_kept_score_nms01):
                proposals_adgtnms.append({'score': score2.item(), 'box': box2.tolist(), 'cls_idx': cls_idx})
            """    
    ...
    return (exp_to_proposals_adgt01,exp_to_proposals_adgtall,exp_to_proposals_adgtnms)
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_sipsep_adgt01_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_sipsep_adgtall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path2 = '/data1/wj/ref_nms/cache/my_sipsep_adgtnms_proposals_{}_{}.pkl'.format(args.m, args.dataset)
