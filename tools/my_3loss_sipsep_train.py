...
def compute_loss(predictor, criterion, device, enable_grad, roi_feats, roi_labels,labels_gt_neg, word_feats, sent_len,sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        scores_gt_neg,ref_scores,fuse_scores, = predictor.forward(roi_feats, packed_sent_feats,sent_feats ,lfeats)  # max_ref_score, max_idx, cls_score, bbox_pred
        ...
        loss2= criterion(fuse_scores.flatten(), roi_labels.flatten())
    return loss
...
def main(args):
    ...
    """
    gt_params =   list(predictor.vis_a_fc1.parameters())+list(predictor.se_img.parameters())\
                 +list(predictor.rank_fc_gt.parameters())
    ref_params = list(predictor.att_fc.parameters()) + list(predictor.rank_fc.parameters()) \
                 + list(predictor.vis_a_fc.parameters()) + list(predictor.vis_r_fc.parameters())
    
    head_params1 = predictor.head1.parameters()   
    fuse_params = predictor.rank_fc_fuse.parameters()      
    ...
    gt_optimizer = optim.Adam(gt_params, lr=CONFIG['GT_LR'], weight_decay=CONFIG['REF_WD'])
    ...
    head1_optimizer = optim.Adam(head_params1, lr=CONFIG['HEAD1_LR'], weight_decay=CONFIG['HEAD_WD'])
    fuse_optimizer = optim.Adam(fuse_params, lr=CONFIG['GT_LR'], weight_decay=CONFIG['HEAD_WD'])
    ...
    gt_scheduler = optim.lr_scheduler.ReduceLROnPlateau(gt_optimizer, min_lr=CONFIG['GT_LR']/100, **common_args)
    ...
    head1_scheduler = optim.lr_scheduler.ReduceLROnPlateau(head_optimizer, min_lr=CONFIG['HEAD1_LR']/100, **common_args)
    fuse_scheduler = optim.lr_scheduler.ReduceLROnPlateau(fuse_optimizer, min_lr=CONFIG['GT_LR']/100, **common_args)
    # Start training
    step = 0
    trn_running_loss = 0.
    ...
    for epoch in range(CONFIG['EPOCH']):
        for trn_batch in trn_loader:
            ...
            if step % VAL_INTERVAL == 0:
                ...
                gt_scheduler.step(avg_val_loss)
                head1_scheduler.step(avg_val_loss)
                fuse_scheduler.step(avg_val_loss)
        ...
        epoch_ckpt = {
            ...
            'head1_optimizer': rnn_optimizer.state_dict(),
            'gt_optimizer': head1_optimizer.state_dict(),
            'fuse_optimizer': fuse_optimizer.state_dict(),
            
            'ref_scheduler': ref_scheduler.state_dict(),
            'rnn_scheduler': rnn_scheduler.state_dict(),
            'head_scheduler': head_scheduler.state_dict(),
            'head1_scheduler': ref_scheduler.state_dict(),
            'gt_scheduler': rnn_scheduler.state_dict(),
            'fuse_scheduler': head_scheduler.state_dict(),
    ...
    save_path = '/home_data/wj/ref_nms/output/my_3loss_sipsep_att_vanilla_{}_{}_b.pth'.format(dataset_splitby,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_3loss_sipsep_att_vanilla_{}_{}.json'.format(args.dataset,tid), 'w') as f:
