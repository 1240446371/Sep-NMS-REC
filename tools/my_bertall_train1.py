...
from lib.my_bert_vanilla_utils import DetBoxDataset
#from lib.my_qkad_predictor import AttVanillaPredictorV2
#from lib.my_bertall_predictor import AttVanillaSigsepPredictorV2
...
def init_att_vanilla_predictor(predictor):
    ...
    for name, param in predictor.named_parameters():
        ...
        if 'head' in name:
            ...
            continue
        if 'weight' in name: 
            #if  name == 'mapping_word.1.weight' or name == 'mapping_word.5.weight' :
            if  'mapping' in name or 'norm' in name :
...
def compute_loss(predictor, criterion1, criterion2, criterion3,device, enable_grad, roi_feats, roi_labels,labels_gt_neg, sent_len,word_feats, sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        #print("104................sentlen%s"%(sent_len,))
        gt_scores,ref_scores = predictor.forward(roi_feats,sent_len, word_feats, sent_feats ,lfeats)  # max_ref_score, max_idx, cls_score, bbox_pred
        #print("78maxscoresshape %s"%scores.shape)
        #labels_gt_neg=labels_gt_neg.to(device)
        #loss0= criterion(score_gt.flatten(), labels_gt.flatten())
        loss1 = criterion1(gt_scores.flatten(), labels_gt_neg.flatten())
        loss2 = criterion2(gt_scores.flatten(), roi_labels.flatten())
        loss3 = criterion3(ref_scores.flatten(), roi_labels.flatten())
        loss=loss1+(loss2)/3+loss3
...
def main(args):
    ...
    predictor = AttVanillaSigadPredictorV2(att_dropout_p=CONFIG['ATT_DROPOUT_P'],
    ...
    ref_params = list(predictor.att_fc.parameters()) + list(predictor.rank_fc.parameters()) \
                 ...
                 + list(predictor.mapping_word.parameters())+ list(predictor.mapping_sent.parameters())
    ...
    for epoch in range(CONFIG['EPOCH']):
        for trn_batch in trn_loader:
            ...
            predictor.train()
            #print("180.....")
            loss = compute_loss(predictor, criterion1,criterion2,criterion3, device, True, *trn_batch)
            ...
            if step % VAL_INTERVAL == 0:
                ...
                for val_batch in val_loader:
                    loss = compute_loss(predictor, criterion1,criterion2,criterion3,device, False, *val_batch)
    ...
    save_path = '/home_data/wj/ref_nms/output/my_bertall_qkad_sigad_{}_{}_b.pth'.format(dataset_splitby,tid)
    #save_path = '/home/wj/code/ref_nms/output/pos06att_vanilla_yb004_{}_{}_b.pth'.format(args.dataset,tid)
    #save_path = '/home/wj/code/ref_nms/output/my_bert_attvanilla_{}_{}_b.pth'.format(args.dataset,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_bertall_qkad_sigad_{}_{}.json'.format(args.dataset,tid), 'w') as f:
