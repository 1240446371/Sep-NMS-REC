...
def rank_proposals(position, gpu_id, tid, refdb_path, split, m,dataset_splitby ):
   ...
   # device = torch.device('cuda', gpu_id)
    ...
    # my coco 
    coco_out_path = "/data1/wj/ref_nms/cache/refcoco+_unc_coco_ctx_out_sent.json" 
    with open(coco_out_path, 'r') as f:
    ...
    for exp_id,pos_box, pos_score, pos_feat, sent_len, bert_token_feats,bert_sent_feats, lfeats, cls_num_list in tqdm_loader:
        ...
        for cls_gt, cls_ref, cls_pos_box, cls_pos_score in zip(gt_score_list, rank_score_list,  pos_box_list, pos_score_list):
            #print("144ok")
            cls_idx += 1
            if  cls_gt.size(0) == 0 : # size(0)some is 0, and some is 16 or other
                continue
            # gt cls
            #print(coco_out[split].keys())
            #print("164...........%s %s"%(type(exp_id),exp_id))
            if str(exp_id) not in coco_out[split].keys() :  # coco in---> cls is import, cls * gt(1/3)  or (cls)^(2) * gt
                #print("162...not in .%s")
            ...
            else:  # coco out, gt is import cls**(1/3)*gt or cls**(1/2)*gt   or (cls* gt**(2)) or (cls* gt)
                #print("173.esp id ..................in")
                ...
                for box1, score1 in zip(cls_kept_box_gt, cls_kept_score_gt):
                    proposals_gt1.append({'score': score1.item(), 'box': box1.tolist(), 'cls_idx': cls_idx})
                    proposals_gt2.append({'score': score1.item(), 'box': box1.tolist(), 'cls_idx': cls_idx})
                    
                
            if str(exp_id) not in coco_out[split].keys() :  # coco in---> cls is import
                final_score_gt2 = ((cls_gt)**(1/2)) * cls_pos_score
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_Sigad_cocoin_gtcbrt_bertall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_Sigad_cocoin_gtsqrt_bertall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    #save_path2 = '/data1/wj/ref_nms/cache/my_Sigad_adatall_bertall_proposals_{}_{}.pkl'.format(args.m, args.dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tid', type=str, default='1113160151')# coco+ 0112095046 0318182906
