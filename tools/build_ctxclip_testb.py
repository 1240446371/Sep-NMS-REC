...
#noun tokens is : ['girl', 'right']
    for split in ( EVAL_SPLITS_DICT[dataset_splitby]):
...
#.397299999997, 'iscrowd': 0, 'image_id': 579382, 'bbox': [416.73, 78.27, 63.27, 447.12], 'category_id': 1, 'id': 201995}
                ...
                for ann in refer.imgToAnns[image_id]:  #ann has many 7 annotations include cat_id 
                    #print("96ann%s"%ann)
                    #print("103 len refer.imgToAnns[image_id]%s"%len(refer.imgToAnns[image_id]))
                    #print("104 refer.imgToAnns[image_id]%s"%(refer.imgToAnns[image_id],))
                    #print("100lenann%s"%len(refer.imgToAnns[image_id]))
                    
                    #ann_glove_list = cat_id_to_glove[ann['category_id']]  # ann box's cat to glove
                    #print("build 105ann glove len %s"%len(ann_glove_list))
                    #print("106 noun len %s"%len(noun_glove_list))
                    ...
                    for token in noun_tokens:
                        ...
                        logit_scale,box_feature,token_feat,logits_per_image, per_text = jit_model(roi_input, re)
                        box_feature,token_feat = torch.squeeze(box_feature),torch.squeeze(token_feat)
                        #print("box shape %s token feat shape %s"%(box_feature.shape,token_feat.shape))
                        cos = torch.cosine_similarity(box_feature,token_feat,dim=0) 
                        coslist=[CAT_ID_TO_NAME[ann['category_id']],token,float(cos)]
                        with open(save_path, 'a') as f:
                              json.dump(coslist, f)
                        print("box class %s and  token%s  cos sim is  %s"%(CAT_ID_TO_NAME[ann['category_id']],token,cos))
                        clip_cos_list.append (cos )
                    """re = clip.tokenize(noun_tokens).to(device_cuda)
                    #print("126 re %s"%re)
                    logit_scale,box_feature,token_feats,logits_per_image, per_text = jit_model(roi_input, re)
                    box_feature,token_feats = torch.squeeze(box_feature),torch.squeeze(token_feats)
                    print("box shape %s token feat shape %s"%(box_feature.shape,token_feats.shape))  
                    clip_cos_list= [torch.cosine_similarity(box_feature,token_feat,dim=0) for token_feat in token_feats] """
                    #print("124 clip cos %s"%clip_cos_list)
                    # caculate a ann_cat glove and noun_glove sim,and >0.4 is cxt box
                                    
                    #print("100cos_sim_list%s"%(cos_sim_list,))
                    # print(CAT_ID_TO_NAME[ann['category_id']], cos_sim_list)
                    max_cos_sim = max(clip_cos_list, default=0.)
                    #print("102max_cos_sim%s"%max_cos_sim)
                    #print("131max_cos_sim%s"%(max_cos_sim))
                    if max_cos_sim > 0.24:  # threshold
                        ann_box = xywh_to_xyxy(ann['bbox'])
                        if calculate_iou(ann_box, gt_box) > 0.9:  # if iou>0.9 is gt,else is 
                            gt_hit = True
                        else:
                            ctx_list.append({'box': ann_box, 'cat_id': ann['category_id']})
                print("151 ctx_list len%s"%len(ctx_list))
                if not gt_hit:
                    gt_miss_num += 1
                if not ctx_list:
                    empty_num += 1
        ...
        save_path1  = '/data1/wj/ref_nms/cache/clip_ctx_info{}.json'.format(dataset_splitby)
    ...
    # Save results
    save_path = '/data1/wj/ref_nms/cache/clip_ctxdb_{}.json'.format(dataset_splitby)
