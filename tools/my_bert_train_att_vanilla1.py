...
from lib.my_bert_vanilla_utils1 import DetBoxDataset
...
def init_att_vanilla_predictor(predictor):
    ...
    for name, param in predictor.named_parameters():
        ...
        if 'weight' in name:
            ...
            if  'mapping_word' in name or 'norm' in name :
...
def compute_loss(predictor, criterion, device, enable_grad, roi_feats, roi_labels,labels_gt_neg, sent_len, word_feats, sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        ref_scores = predictor.forward(roi_feats,sent_len, word_feats, sent_feats ,lfeats)  # max_ref_score, max_idx, cls_score, bbox_pred
...
def main(args):
    ...
    ref_params = list(predictor.att_fc.parameters()) + list(predictor.rank_fc.parameters()) \
                 ...
                 + list(predictor.mapping_word.parameters())
    ...
    save_path = '/home_data/wj/ref_nms/output/my_bert_attvanilla_{}_{}_b.pth'.format(dataset_splitby,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_bert_attvanilla_{}_{}.json'.format(args.dataset,tid), 'w') as f:
