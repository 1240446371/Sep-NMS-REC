...
# txt contain: sentence id,re id, re's all token id , gt box
def build_refdb(dataset, split_by):
    # Load refer data
    refer = REFER('/home_data/wj/ref_nms/data/refer', dataset, split_by)
    # Load vocab
    with open('/home_data/wj/ref_nms/cache/std_vocab_{}_{}.txt'.format(dataset, split_by)) as f:
        idx_to_wd = [wd[:-1] for wd in f.readlines()]  # trim off newline
        print("22 idx_to_wd %s"%idx_to_wd)
    wd_to_idx = {}
    for idx, wd in enumerate(idx_to_wd):
        wd_to_idx[wd] = idx
        print("26 wd_to_idx %s"%wd_to_idx)
    # Build refdb
    ...
    refdb_save_path = '/home_data/wj/ref_nms/cache/std_refdb_{}.json'.format(dataset_splitby)
