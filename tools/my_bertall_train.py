...
def compute_loss(predictor, criterion1, criterion2, device, enable_grad, roi_feats, roi_labels,labels_gt_neg, sent_len,word_feats, sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        loss2 = criterion2(ref_scores.flatten(), roi_labels.flatten())
        loss=loss1+loss2
...
def main(args):
    ...
    predictor = AttVanillaSigadnolocPredictorV2(att_dropout_p=CONFIG['ATT_DROPOUT_P'],
    ...
    for epoch in range(CONFIG['EPOCH']):
        for trn_batch in trn_loader:
            ...
            predictor.train()
            #print("180.....")
            loss = compute_loss(predictor, criterion1,criterion2, device, True, *trn_batch)
            ...
            if step % VAL_INTERVAL == 0:
                ...
                for val_batch in val_loader:
                    loss = compute_loss(predictor, criterion1,criterion2,device, False, *val_batch)
    ...
    save_path = '/home_data/wj/ref_nms/output/my_bertall_qkad_sigad_noloc_{}_{}_b.pth'.format(dataset_splitby,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_bertall_qkad_sigad_noloc{}_{}.json'.format(args.dataset,tid), 'w') as f:
