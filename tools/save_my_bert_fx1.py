...
def rank_proposals(position, gpu_id, tid, refdb_path, split, m,dataset_splitby ):
   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,pos_box, pos_score, pos_feat, sent_len, bert_token_feats,bert_sent_feats, lfeats, cls_num_list in tqdm_loader:
        ...
        idx = np.where(gt_score> 0.01)
        gt_score1 = np.where(gt_score>0.01,gt_score,0) # 0.01 ->0 all
        print("143......gtscore%s"%(gt_score1.shape,))
        gt_score_fx01 = np.where(gt_score>0.01,gt_score,0)
        gt_score_fx011 = np.where(gt_score>0.01,gt_score,0)
        gt_score_fx02 = np.where(gt_score>0.01,gt_score,0) 
        #gt_score = torch.where
   
        #print("147..............idx%s"%idx)       
        gt_score2 = gt_score[idx] # fx  
        gt_score22 = gt_score[idx]
        gt_score222 = gt_score[idx]   
        #print("150..............gt_score2%s"%(gt_score2.shape,))      
        gt_score_fx1 = gt_score2
        gt_score_fx11 = gt_score22
        gt_score_fx2 = gt_score222       
        

        
        #print("lenidx%s"%len(idx))
        #pos_box=pos_box[idx]
        #print("156....posbox%s"%(pos_box.shape,))
        """
        ...
        for i in cats1.codes:
           if i ==0:
             ...
             gt_score_fx1[j]=0.1
             #print("186....gt_score_fx1%s"%gt_score_fx1)
           if i ==1:
             gt_score_fx1[j]=0.2
           if i ==2:
             gt_score_fx1[j]=0.3
           ...
           j+=1     
        jj=0
        #print("206............cats1%s"%(cats1.codes,))
        for ii in cats1.codes:
           if ii ==0:
             gt_score_fx11[jj]=0.5
           if ii ==1:
             #print("210...........labele =1")
             gt_score_fx11[jj]=0.55
           if ii ==2:
             gt_score_fx11[jj]=0.6
           if ii ==3:
             gt_score_fx11[jj]=0.65
           if ii ==4:
             gt_score_fx11[jj]=0.7
           if ii ==5:
             gt_score_fx11[jj]=0.75            
           if ii ==6:
             gt_score_fx11[jj]=0.8
           if ii ==7:
             gt_score_fx11[jj]=0.85
           if ii ==8:
             gt_score_fx11[jj]=0.9
           if ii ==9:
             gt_score_fx11[jj]=0.95
           jj+=1
            
                  
        labels2=['0', '1', '2','3','4','5']
        cats2 = pd.qcut(gt_score_fx2, 6 ,duplicates='drop',labels= labels2)
        #print("214............")
        #print("233............cats2%s"%(cats2.codes,))
        m=0
        ...
        #print("249  ......gt_score_fx2%s"%(gt_score_fx2.shape,))
        #print("253.......gt_score_fx1%s"%gt_score_fx1)
        #print("254.......gt_score_fx11%s"%gt_score_fx11)  
        #print(gt_score_fx1==gt_score_fx11)
        gt_score_fx01[idx] = gt_score_fx1  
        gt_score_fx011[idx] = gt_score_fx11
        gt_score_fx02[idx] = gt_score_fx2 
        #print("253......gt_score_fx01%s"%(gt_score_fx01.shape,))  
        #print("254gt_score_fx01%s"%gt_score_fx01)
        #print("255 gt_score_fx011%s"%gt_score_fx011)
        #print(gt_score_fx01==gt_score_fx011)
        ...
        gt_score_fx01=torch.from_numpy(gt_score_fx01).to(device)
        gt_score_fx011=torch.from_numpy(gt_score_fx011).to(device)
        gt_score_fx02=torch.from_numpy(gt_score_fx02).to(device)

        #gt_score_fx3=torch.from_numpy(gt_score_fx3).to(device)
        
        gt_fx1_list = torch.split(gt_score_fx01, cls_num_list, dim=0)  # tuple    
        gt_fx11_list = torch.split(gt_score_fx011, cls_num_list, dim=0)  # tuple    
        gt_fx2_list = torch.split(gt_score_fx02, cls_num_list, dim=0)  # tuple  
        ...
        cls_idx = 0
        for cls_gt1,cls_gt11, cls_gt2,cls_pos_box, cls_pos_score in zip(gt_fx1_list, gt_fx11_list, gt_fx2_list,pos_box_list, pos_score_list):
            #print("311...............ok")
            cls_idx += 1
            if cls_gt1.size(0) == 0 and  cls_gt11.size(0) == 0 and cls_gt2.size(0) == 0 : # size(0)some is 0, and some is 16 or other
                continue
            # gt cls
    ...
    return (exp_to_proposals_gt1,exp_to_proposals_gt11 ,exp_to_proposals_gt2)
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_Sigad_gtfxqcut(0.01_10)_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_Sigad_gtfxqcut(0.01_0.5_10)_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path2 = '/data1/wj/ref_nms/cache/my_Sigad_gtfxqcut(0.01_0.5_6)_proposals_{}_{}.pkl'.format(args.m, args.dataset)
