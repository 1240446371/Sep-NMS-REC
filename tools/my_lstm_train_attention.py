...
class BCEFocalLoss(torch.nn.Module):
    ...
    def __init__(self, gamma=1, alpha=0.5, reduction='elementwise_mean'):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha
...
def compute_loss(predictor,criterion_ref, criterion_gt,device, enable_grad, roi_feats, roi_labels,labels_gt_neg, word_feats, sent_len,sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        #print("129scores_gt_neg %s"%type(scores_gt_neg))
        #print("130ref_score %s"%type(ref_scores))
        loss1= criterion_gt(scores_gt_neg.flatten(), labels_gt_neg.flatten())
        print("132lossgt.......%s"%loss1)
        loss = criterion_ref(ref_scores.flatten(), roi_labels.flatten())
        print("134lossref.......%s"%loss)
    return loss+loss1
...
def main(args):
    ...
    predictor.to(device)
    # Setup loss
    criterion_ref = nn.BCEWithLogitsLoss(reduction='mean')
    criterion_gt = nn.BCEWithLogitsLoss(reduction='mean')
    # Setup optimizer
    """
    ...
    ref_params = list(predictor.att_fc.parameters()) + list(predictor.rank_fc.parameters()) \
                 + list(predictor.vis_a_fc.parameters()) + list(predictor.vis_r_fc.parameters())\
                 +list(predictor.vis_a_fck.parameters())+list(predictor.mapping_sent.parameters())
    #print("131........")
    ...
    save_path = '/home_data/wj/ref_nms/output/my_1sipqk_ad_att_vanilla_{}_{}_b.pth'.format(dataset_splitby,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_1sipqk_ad_att_vanilla_{}_{}.json'.format(args.dataset,tid), 'w') as f:
