...
def build_ctxdb(dataset, split_by):
    ...
    device_cuda=torch.device('cuda',3)
    
    clip_model = CLIP_MODEL(512, 'CLIP_FT',device_cuda)
    state_dict = \
    ...
    IMAGE_DIR ='/data1/Source/dataset/cv/COCO_images'
    # Load GloVe feature
...
#.397299999997, 'iscrowd': 0, 'image_id': 579382, 'bbox': [416.73, 78.27, 63.27, 447.12], 'category_id': 1, 'id': 201995}
                ...
                for ann in refer.imgToAnns[image_id]:  # ann has many 7 annotations include cat_id 
                    ...
                    ann_cat_list.append(ann['category_id'])
                ...
                for item in clip_soft:
                    ...
                    if item>=0.08:
                        ...
                        if calculate_iou(ann_box_list[i], gt_box) > 0.9:  # if iou>0.9 is gt,else is 
                            ...
                            ctx_list.append({'box': ann_box_list[i], 'cat_id': ann_cat_list[i]})
                    i+=1
        ...
        #save_path1  = '/data1/wj/ref_nms/cache/ybclip_ctx_06seninfo{}.json'.format(dataset_splitby)
        save_path1  = '/home_data/wj/ref_nms/cache/yb_ctx_08seninfo{}.json'.format(dataset_splitby)
        #save_path1  = '/data1/wj/ref_nms/cache/test37.json'
    ...
    # Save results
    save_path = '/home_data/wj/ref_nms/cache/yb_ctxdb_ann_08sen{}.json'.format(dataset_splitby)
    #save_path = '/data1/wj/ref_nms/cache/test3701.json'
...
def main():
    ...
    for dataset, split_by in [ ('refcoco', 'unc')]:
