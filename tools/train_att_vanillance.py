...
from lib.vanilla_utilsnce import DetBoxDataset
from lib.predictornce import AttVanillaPredictorV2
...
def compute_loss(predictor, criterion, device, enable_grad, roi_feats, gt_num,ctx_pos_num, neg_num, roi_labels, labels_gt,labels_gt_ctx, word_feats, sent_len):
    with torch.autograd.set_grad_enabled(enable_grad):
        #print("roi_feats%s"%(roi_feats.shape,))
        #print("roi_labels%s"%(roi_labels.shape,))
        #print("77gt_num,%s"%(gt_num.shape,))
        #print("78ctx_pos_num,%s"%(ctx_pos_num.shape,))
        #print(" 79neg_num %s"% (neg_num.shape,))
        ...
        score_gt,score_gt_ctx,scores, *_ = predictor.forward(roi_feats, gt_num,ctx_pos_num, neg_num, packed_sent_feats)
        #print("78maxscoresshape %s"%(scores.shape,))
        #print("81roi_labels %s"%(roi_labels.device))
        #print("81score_gt %s labels_gt %s"%(score_gt.shape,labels_gt.shape,)) 
        #print("90score_gt_ctx %s labels_gt_ctx%s"%(score_gt_ctx.shape,labels_gt_ctx.shape,))
        #print("91score_gt %s labels_gt %s"%(score_gt.device,labels_gt.device,)) 
        #print("92score_gtctx %s labels_gt_ctx%s"%(score_gt_ctx.device,labels_gt_ctx.device))
        labels_gt=labels_gt.to(device)
        labels_gt_ctx=labels_gt_ctx.to(device)
        loss0= criterion(score_gt.flatten(), labels_gt.flatten())
        loss1= criterion(score_gt_ctx.flatten(), labels_gt_ctx.flatten())
        loss = criterion(scores.flatten(), roi_labels.flatten())
        loss_all = loss0+loss1+loss
        print("loss0 %s loss1 %s  "%(loss0.item(),loss1.item()))
...
def main(args):
    ...
    with open(refdb_path, 'r') as f:
        refdb = json.load(f)
    #ctxdb_path =   "/data1/wj/ref_nms/cache/ybclip_ctxdb_ann_01senrefcoco_unc.json"
    ...
    for epoch in range(CONFIG['EPOCH']):
        ...
        #save_path = '/home_data/wj/ref_nms/output/nce{}_{}.pth'.format(dataset_splitby, epoch + 1)
        #save_path = '/home_data/wj/ref_nms/output/01ybclipnce_t0007t101_{}_{}.pth'.format(dataset_splitby, epoch + 1)
        save_path = '/data1/wj/ref_nms/output/refnce_t0005t1007_{}_{}.pth'.format(dataset_splitby, epoch + 1)
    ...
    #save_path = '/home_data/wj/ref_nms/output/refnce_{}_b.pth'.format(dataset_splitby)
    #save_path = '/home_data/wj/ref_nms/output/01ybclipnce_t0007t101_{}_b.pth'.format(dataset_splitby)
    save_path = '/data1/wj/ref_nms/output/refnce_t0005t1007_{}_b.pth'.format(dataset_splitby)
    ...
    #json_pth = '/home_data/wj/ref_nms/output/refnce_{}.json'.format(dataset_splitby)
    #json_pth = '/home_data/wj/ref_nms/output/01ybclipnce_t0007t101_{}.json'.format(dataset_splitby)
    json_pth = '/data1/wj/ref_nms/output/refnce_t0005t1007_{}.json'.format(dataset_splitby)
