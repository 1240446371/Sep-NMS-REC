   ...
   # device = torch.device('cuda', gpu_id)
    ...
    for exp_id,lfeats, pos_feat, sent_feat, bert_feats,pos_box, pos_score, cls_num_list in tqdm_loader:
        ...
        for cls_gt, cls_ref, cls_pos_box, cls_pos_score in zip(gt_score_list,rank_score_list,pos_box_list, pos_score_list):
            ...
            gt_score_01 = torch.where(cls_gt < 0.1, zero, cls_gt)
            #cls_pos_score02 = torch.where(cls_pos_score < 0.2,zero,cls_pos_score)
            keepgt = nms(cls_pos_box, gt_score_01, iou_threshold=0.3)  # >0.1 and nms
            
            final_score0= cls_gt * cls_pos_score
            final_score= torch.where(final_score0 < 0.05, zero, final_score0)
            
            keepgtcls = nms(cls_pos_box, final_score, iou_threshold=0.3) # nms 
            keep=torch.cat((keepgt,keepgtcls),dim=0)
            keep = torch.unique(keep,sorted=False)
            print("194keep%s"%keep)


            cls_kept_box_gt = cls_pos_box[keep]
            cls_kept_score_gt = cls_gt[keep]
            ...
            gt_score_005 = torch.where(cls_gt < 0.05, zero, cls_gt)
            keepgt005 = nms(cls_pos_box, gt_score_005, iou_threshold=0.3)  # >0.1 and nms
            
            keep1=torch.cat((keepgt005,keepgtcls),dim=0)
            keep1 = torch.unique(keep,sorted=False)
            #print("194keep%s"%keep)


            cls_kept_box_gt1 = cls_pos_box[keep1]
            cls_kept_score_gt1 = cls_gt[keep1]
...
def main(args):
    ...
    save_path0 = '/data1/wj/ref_nms/cache/my_qkad_gt01andcls005_proposals_{}_{}.pkl'.format(args.m, args.dataset)
    save_path1 = '/data1/wj/ref_nms/cache/my_qkad_gt005andcls005_proposals_{}_{}.pkl'.format(args.m, args.dataset)
