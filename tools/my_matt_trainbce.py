...
from lib.my_mattloc_predictor import AttVanillaPredictorV2
...
def init_att_vanilla_predictor(predictor):
    ...
    for name, param in predictor.named_parameters():
        ...
        if 'head' in name:
            ...
            continue
        if 'weight' in name:
            #print("71 name %s ,param%s"%(name,param.shape,))
            if  name == 'mapping_sent.1.weight' or name == 'mapping_sent.5.weight' or  'vis_emb_fc' in  name   or 'lang_emb_fc' in name:
...
def compute_loss(predictor, criterion_ref, criterion_gt,device, enable_grad, roi_feats, roi_labels,labels_gt_neg, word_feats, sent_len,sent_feats,lfeats):
    with torch.autograd.set_grad_enabled(enable_grad):
        ...
        #log_ref=torch.log(ref_scores)
        #print("ref_scores.shape%s"%(ref_scores.shape,))
        #print("ref_scoresflatten.shape%s"%((ref_scores.flatten()).shape,))
        loss_gt= criterion_gt(scores_gt_neg.flatten(), labels_gt_neg.flatten())
        #loss_ref = criterion_ref(log_ref.flatten(), roi_labels.flatten())
        loss_ref = criterion_ref(ref_scores.flatten(), roi_labels.flatten())
    return loss_gt+loss_ref
...
def main(args):
    ...
    trn_loader = DataLoader(trn_dataset, batch_size=CONFIG['BATCH_SIZE'], shuffle=True, num_workers=2)
    ...
    criterion_ref = nn.BCELoss(reduction='mean')
    criterion_gt = nn.BCELoss(reduction='mean')
    ...
    ref_params = list(predictor.att_fc.parameters()) + list(predictor.rank_fc.parameters()) \
                 ...
                 +list(predictor.mapping_sent.parameters())\
                 + list(predictor.vis_emb_fc.parameters())+ list(predictor.lang_emb_fc.parameters())\
                 + list(predictor.attn_fuse.parameters()) 
    #print("131........")
    ...
    for epoch in range(CONFIG['EPOCH']):
        for trn_batch in trn_loader:
            ...
            predictor.train()
            #print("180.....")
            loss = compute_loss(predictor, criterion_ref,criterion_gt, device,True, *trn_batch)
            ...
            if step % VAL_INTERVAL == 0:
                ...
                for val_batch in val_loader:
                    loss = compute_loss(predictor, criterion_ref,criterion_gt, device, False, *val_batch)
        ...
        epoch_time=int(time() - tic) // 60
        print("262 time 1 epoch %s"%(epoch_time,))
    ...
    save_path = '/home_data/wj/ref_nms/output/my_bcemattloc_ad_{}_{}_b.pth'.format(dataset_splitby,tid)
    #save_path = '/home/wj/code/ref_nms/output/pos06att_vanilla_yb004_{}_{}_b.pth'.format(args.dataset,tid)
    #save_path = '/home/wj/code/ref_nms/output/my_matt_ad_{}_{}_b.pth'.format(args.dataset,tid)
    ...
    with open('/home_data/wj/ref_nms/output/my_bcemattloc_ad_{}_{}.json'.format(args.dataset,tid), 'w') as f:
